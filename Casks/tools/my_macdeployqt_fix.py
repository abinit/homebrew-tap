...
# -*- coding: utf-8 -*-
"""
This script is a fork from https://github.com/arl/macdeployqtfix
  i(author: Aurélien Rainone) with several improvements
...
QTLIB_NAME_REGEX_1 = r'^(?:@executable_path)?/.*/(Qt[a-zA-Z]*).framework/(?:Versions/\S/)?\1$'
QTLIB_NAME_REGEX_2 = r'^(?:@rpath)?/(Qt[a-zA-Z]*).framework/(?:Versions/\S/)?\1$'
...
homebrewpath = os.popen('brew --prefix').read().strip()
BREWLIB_REGEX_1 = r'^$homebrewpath/.*/(.*)'
BREWLIB_REGEX_2 = '^@loader_path/\.\./\.\./\.\./\.\./opt/.*/lib/(.*)'
BREWLIB_NORMALIZED = r'$prefix/Frameworks/$brewlib'

RELATIVE_LIB_REGEX = r'^@loader_path/(.*)'
RELATIVE_LIB_NORMALIZED = r'$prefix/$relativelib'
...
def is_qt_lib(filename):
    ...
    Accepts absolute path as well as path containing @executable_path
    """
    qtlib_name_rgx_1 = re.compile(QTLIB_NAME_REGEX_1)
    qtlib_name_rgx_2 = re.compile(QTLIB_NAME_REGEX_2)
    match_1 = qtlib_name_rgx_1.match(filename)
    match_2 = qtlib_name_rgx_2.match(filename)
...
def is_brew_lib(filename):
    ...
    Accepts absolute path as well as path containing @executable_path
    """
    brewlib_name_rgx_1 = re.compile(BREWLIB_REGEX_1)
    brewlib_name_rgx_2 = re.compile(BREWLIB_REGEX_2)
    match_1 = brewlib_name_rgx_1.match(filename)
    match_2 = brewlib_name_rgx_2.match(filename)
    return (match_1 is not None) or (match_2 is not None)
...
def normalize_qtplugin_name(filename):
    ...
    output:
        a tuple (qtlib, abspath, rpath) where:
            ...
            - relpath is the correct rpath to the qt lib inside the app bundle
    """
...
def normalize_qtlib_name(filename):
    ...
    input: a path to a qt library, as returned by otool, that can have this form :
            ...
            - @rpath/QtSerialPort.framework/Versions/5/QtSerialPort
    output:
        a tuple (qtlib, abspath, rpath) where:
    ...
    qtlib_name_rgx_1 = re.compile(QTLIB_NAME_REGEX_1)
    qtlib_name_rgx_2 = re.compile(QTLIB_NAME_REGEX_2)
    rgxret_1 = qtlib_name_rgx_1.match(filename)
    rgxret_2 = qtlib_name_rgx_2.match(filename)
    if (not rgxret_1) and (not rgxret_2):
        msg = 'couldn\'t normalize a qt lib filename: {0}'.format(filename)
        GlobalConfig.logger.critical(msg)
        raise Exception(msg)
    ...
    if rgxret_1:
        qtlib = rgxret_1.groups()[0]
    else:
        qtlib = rgxret_2.groups()[0]
...
def normalize_brew_name(filename):
    ...
    input: a path to a brew library, as returned by otool, that can have this form :
            - an absolute path homebrew_prefix/lib/yyy
            - an relative path @loader_path/../../../../opt/xxx/lib/yyy
    output:
        a tuple (brewlib, abspath, rpath) where:
            - brewlib is the name of the brew lib
            - abspath is the absolute path of the lib inside the app bundle of exepath
            - relpath is the correct rpath to the lib inside the app bundle
    """
    GlobalConfig.logger.debug('normalize_brew_name({0})'.format(filename))

    brewlib_name_rgx_1 = re.compile(BREWLIB_REGEX_1)
    brewlib_name_rgx_2 = re.compile(BREWLIB_REGEX_2)
    rgxret_1 = brewlib_name_rgx_1.match(filename)
    rgxret_2 = brewlib_name_rgx_2.match(filename)
    if (not rgxret_1) and (not rgxret_2):
    ...
    if rgxret_1:
        brewlib = rgxret_1.groups()[0]
    else:
        brewlib = rgxret_2.groups()[0]

    # from brewlib, forge 2 paths:
    #  - absolute path of lib in bundle,
    abspath = os.path.normpath(templ.safe_substitute(
        prefix=os.path.dirname(GlobalConfig.exepath) + '/..',
        brewlib=brewlib))
...
def normalize_relative_name(filename):
    """
    input: a path to a library, as returned by otool, that can have this form :
            - an relative path @loader_path/yyy
    output:
        a tuple (relativelib, abspath, rpath) where:
            - relativelib is the name of the lib
            - abspath is the absolute path of the lib inside the app bundle of exepath
            - relpath is the correct rpath to the lib inside the app bundle
    """
    GlobalConfig.logger.debug('normalize_relative_name({0})'.format(filename))

    relative_name_rgx = re.compile(RELATIVE_LIB_REGEX)
    rgxret = relative_name_rgx.match(filename)
    if not rgxret:
        msg = 'couldn\'t normalize a relative lib filename: {0}'.format(filename)
        GlobalConfig.logger.critical(msg)
        raise Exception(msg)

    # relative lib normalization settings
    templ = Template(RELATIVE_LIB_NORMALIZED)
    relativelib = rgxret.groups()[0]

    # from relativelib, forge 2 paths:
    #  - absolute path of lib in bundle,
    abspath = os.path.normpath(templ.safe_substitute(
        prefix=os.path.dirname(GlobalConfig.exepath) + '/../Frameworks',
        relativelib=relativelib))

    #  - and rpath containing @loader_path, relative to libpath
    rpath = templ.safe_substitute(
        prefix='@loader_path',
        relativelib=relativelib)

    GlobalConfig.logger.debug('\treturns({0})'.format((relativelib, abspath, rpath)))
    return relativelib, abspath, rpath
...
def fix_dependency(binary, dep):
    ...
    if is_qt_lib(dep):
        ...
        qtname, dep_abspath, dep_rpath = normalize_relative_name(dep)
    ...
    if dep_ok and not os.path.exists(dep_abspath):
        ...
        if proc_out.retcode != 0:
            ...
            proc_out_qt = run_and_get_output(popen_args)
            # from Homebrew location
            if proc_out_qt.retcode != 0:
                hbnamesrc = os.path.join(GlobalConfig.hbpath, 'lib', '{0}'.format(qtname))
                popen_args = ['cp', hbnamesrc, dep_abspath]
                proc_out_hb = run_and_get_output(popen_args)
                if proc_out_hb.retcode != 0:
                    GlobalConfig.logger.info(proc_out_qt.stderr)
                    GlobalConfig.logger.info(proc_out_hb.stderr)
                    dep_ok = False
        if dep_ok:
...
def main():
 ...
 """
    ...
    GlobalConfig.hbpath = os.path.normpath(homebrewpath)
